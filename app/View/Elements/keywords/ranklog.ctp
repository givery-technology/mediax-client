                ...
                <table class="table table-striped table-bordered bootstrap-datatable datatable dataTable">
                    <thead>
                        <tr role="row">
                            ...
                            <th style="width: auto;" colspan="1" rowspan="1" class=""><?php echo __('Rank'); ?></th>
                    ...
                    <?php foreach ($data_rankhistories as $rankhistory):  $rank = json_decode($rankhistory['Ranklog']['rank'], true);?>
	                    ...
	                    <tbody aria-relevant="all" aria-live="polite" role="alert">
	                        <tr class="">
	                            <td class="sorting_1"><?php echo date('Y-m-d', strtotime($rankhistory['Ranklog']['rankdate'])); ?>&nbsp;</td>
	<!--Rank-->
	                            <td  class="center">
...
<?php
	...
	foreach ($rankhistories as $rankhistory): $rank = json_decode($rankhistory['Ranklog']['rank'], true);
		$graph_date[] = "'" .$rankhistory['Ranklog']['rankdate'] ."'";
...
<!--Graph: Date Range date-->
                <?php echo $this->Form->create('Ranklog',array('class'=>'form-search')); ?>
                <div class="form-group">
            ...
            $.ajax({
                ...
                success:function(data){
                    ...
                    $.each(data, function(key,val){
                        data_rankDate.push(val.Ranklog.rankdate);
                        data_google_rank.push(parseInt(val.Ranklog.google_jp));
                        data_yahoo_rank.push(parseInt(val.Ranklog.yahoo_jp));
                    })
                    $('#graph').highcharts({
                        title: {
                            text: '<?php echo __("Rank History"); ?>',
